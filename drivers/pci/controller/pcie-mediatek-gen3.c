...
static int mtk_pcie_devices_power_up(struct mtk_gen3_pcie *pcie)
{
	...
	if (!(pcie->soc->flags & SKIP_PCIE_RSTB)) {
		 ...
		 * PCIE_PHY_RSTB is asserted, we have to de-assert MAC & PHY &
		 * BRG reset signals first to allow the REFCLK to be stable.
		 * While PCIE_BRG_RSTB is asserted, there is a short period
		 * during which the PCIe internal register cannot be accessed,
		 * so we need to wait 10ms here.
		 */
