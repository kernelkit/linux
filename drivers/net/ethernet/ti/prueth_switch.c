...
const struct prueth_queue_info sw_queue_infos[][NUM_QUEUES] = {
	...
	[PRUETH_PORT_QUEUE_MII0] = {
		...
		[PRUETH_QUEUE5] = {
			P1_Q5_BUFFER_OFFSET,
			P1_Q5_BUFFER_OFFSET +
				((QUEUE_5_SIZE - 1) * ICSS_BLOCK_SIZE),
			P1_Q5_BD_OFFSET,
			P1_Q5_BD_OFFSET + ((QUEUE_5_SIZE - 1) * BD_SIZE),
		},
		[PRUETH_QUEUE6] = {
			P1_Q6_BUFFER_OFFSET,
			P1_Q6_BUFFER_OFFSET +
				((QUEUE_6_SIZE - 1) * ICSS_BLOCK_SIZE),
			P1_Q6_BD_OFFSET,
			P1_Q6_BD_OFFSET + ((QUEUE_6_SIZE - 1) * BD_SIZE),
		},
		[PRUETH_QUEUE7] = {
			P1_Q7_BUFFER_OFFSET,
			P1_Q7_BUFFER_OFFSET +
				((QUEUE_7_SIZE - 1) * ICSS_BLOCK_SIZE),
			P1_Q7_BD_OFFSET,
			P1_Q7_BD_OFFSET + ((QUEUE_7_SIZE - 1) * BD_SIZE),
		},
	},
	[PRUETH_PORT_QUEUE_MII1] = {
		...
		[PRUETH_QUEUE5] = {
			...
			P2_Q5_BUFFER_OFFSET +
				((QUEUE_5_SIZE - 1) * ICSS_BLOCK_SIZE),
			P2_Q5_BD_OFFSET,
			P2_Q5_BD_OFFSET + ((QUEUE_5_SIZE - 1) * BD_SIZE),
		},
		[PRUETH_QUEUE6] = {
			P2_Q6_BUFFER_OFFSET,
			P2_Q6_BUFFER_OFFSET +
				((QUEUE_6_SIZE - 1) * ICSS_BLOCK_SIZE),
			P2_Q6_BD_OFFSET,
			P2_Q6_BD_OFFSET + ((QUEUE_6_SIZE - 1) * BD_SIZE),
		},
		[PRUETH_QUEUE7] = {
			P2_Q7_BUFFER_OFFSET,
			P2_Q7_BUFFER_OFFSET +
				((QUEUE_7_SIZE - 1) * ICSS_BLOCK_SIZE),
			P2_Q7_BD_OFFSET,
			P2_Q7_BD_OFFSET + ((QUEUE_7_SIZE - 1) * BD_SIZE),
		},
	},
};
...
static const struct prueth_queue_info rx_queue_infos[][NUM_QUEUES] = {
	...
	[PRUETH_PORT_QUEUE_MII0] = {
		...
		[PRUETH_QUEUE5] = {
			P1_Q5_BUFFER_OFFSET,
			P1_QUEUE_DESC_OFFSET + 32,
			P1_Q5_BD_OFFSET,
			P1_Q5_BD_OFFSET + ((QUEUE_5_SIZE - 1) * BD_SIZE),
		},
		[PRUETH_QUEUE6] = {
			P1_Q6_BUFFER_OFFSET,
			P1_QUEUE_DESC_OFFSET + 40,
			P1_Q6_BD_OFFSET,
			P1_Q6_BD_OFFSET + ((QUEUE_6_SIZE - 1) * BD_SIZE),
		},
		[PRUETH_QUEUE7] = {
	...
	[PRUETH_PORT_QUEUE_MII1] = {
		...
		[PRUETH_QUEUE5] = {
			P2_Q5_BUFFER_OFFSET,
			P2_QUEUE_DESC_OFFSET + 32,
			P2_Q5_BD_OFFSET,
			P2_Q5_BD_OFFSET + ((QUEUE_5_SIZE - 1) * BD_SIZE),
		},
		[PRUETH_QUEUE6] = {
			P2_Q6_BUFFER_OFFSET,
			P2_QUEUE_DESC_OFFSET + 40,
			P2_Q6_BD_OFFSET,
			P2_Q6_BD_OFFSET + ((QUEUE_6_SIZE - 1) * BD_SIZE),
		},
		[PRUETH_QUEUE7] = {
			P2_Q7_BUFFER_OFFSET,
			P2_QUEUE_DESC_OFFSET + 48,
			P2_Q7_BD_OFFSET,
			P2_Q7_BD_OFFSET + ((QUEUE_7_SIZE - 1) * BD_SIZE),
		}, 
	},
...
const struct prueth_queue_info lre_queue_infos[][NUM_QUEUES] = {
	...
	[PRUETH_PORT_QUEUE_MII0] = {
		...
		/* Below code is for HSR/PRP Updated queue structure ( Dedicated queues for PTP, SV and GOOSE )
 		 * Roopak@CIT - 02-June-2023
 		 */
		[PRUETH_QUEUE5] = {
			HSRP1_TXOPT_Q5_BUFFER_OFFSET,
			HSRP1_TXOPT_Q5_BUFFER_OFFSET +
				((QUEUE_5_HSRPRP_TXOPT_SIZE - 1) * ICSS_BLOCK_SIZE),
			HSRP1_TXOPT_Q5_BD_OFFSET,
			HSRP1_TXOPT_Q5_BD_OFFSET + ((QUEUE_5_HSRPRP_TXOPT_SIZE - 1) * BD_SIZE),
		},
		[PRUETH_QUEUE6] = {
			HSRP1_TXOPT_Q6_BUFFER_OFFSET,
			HSRP1_TXOPT_Q6_BUFFER_OFFSET +
				((QUEUE_6_HSRPRP_TXOPT_SIZE - 1) * ICSS_BLOCK_SIZE),
			HSRP1_TXOPT_Q6_BD_OFFSET,
			HSRP1_TXOPT_Q6_BD_OFFSET + ((QUEUE_6_HSRPRP_TXOPT_SIZE - 1) * BD_SIZE),
		},
		[PRUETH_QUEUE7] = {
			HSRP1_TXOPT_Q7_BUFFER_OFFSET,
			HSRP1_TXOPT_Q7_BUFFER_OFFSET +
				((QUEUE_7_HSRPRP_TXOPT_SIZE - 1) * ICSS_BLOCK_SIZE),
			HSRP1_TXOPT_Q7_BD_OFFSET,
	...
	*/
	if (PRUETH_IS_LRE(prueth))
		memcpy_toio(dram, hsr_prp_txopt_queue_descs[PRUETH_PORT_QUEUE_HOST],
  		...
  		* Roopak@cit - 24-August-2023
  		*/
		memcpy_toio(dram, sw_queue_descs[PRUETH_PORT_QUEUE_HOST],
			    sizeof(sw_queue_descs[PRUETH_PORT_QUEUE_HOST]));
				 ...
				 enum prueth_port port_id)
{
	...
	*/
	...
	memcpy_toio(dram_base + queue_desc_ofs,
		    &sw_queue_descs[port_id][0],
		    NUM_QUEUES * sizeof(sw_queue_descs[port_id][0]));
