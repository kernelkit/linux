...
void fault_dump_persist(struct fault_callback *fault_dumper)
{
    ...
    total_size_needed = currently_occupied_bytes;
    ...
    if(sram_size < total_size_needed)
        goto exit_persist;

    writel_relaxed(total_size_needed - sizeof(written_bytes) - currently_occupied_bytes, sram_mem);
    ...
    writel_relaxed(total_size_needed, cb_handle->memories_used[DEST_SLOT].memory_base);
